...
export default function MissionList({ 
  ...
  if (missions.length === 0) {
    return <p>{type === 'active' ? 'No active missions.' : 'No completed missions.'}</p>;
  }
  ...
  return (
    <ul className="space-y-4">
      {missions.map((mission) => (
        ...
        >
          <h2 className={`${type === 'active' ? 'text-xl' : 'text-lg'} font-semibold`}>
            {mission.mission.title}
          </h2>
          ...
          <p className={`text-sm ${type === 'active' ? 'mb-2' : 'text-gray-600'}`}>
